...
def reset_position(image_face, total, pcd):
    ...
    #print("$$$$$$$$$", angle_rad)
    if image_pos < 90:
        translation_matrix = np.array([[1, 0, 0, pos_factor * 0.0006 * math.cos(np.pi/2 - angle_rad)], [0, 1, 0, 0], [0, 0, 1, pos_factor * 0.0006 * math.cos(angle_rad)], [0, 0, 0, 1]])
        pcd.transform(translation_matrix)
    elif image_pos < 180:
        translation_matrix = np.array([[1, 0, 0, pos_factor * 0.0006 * math.cos(angle_rad - np.pi/2)], [0, 1, 0, 0], [0, 0, 1, -pos_factor * 0.0006 * math.cos(np.pi - angle_rad)], [0, 0, 0, 1]])
        pcd.transform(translation_matrix)
    elif image_pos < 270:
        translation_matrix = np.array([[1, 0, 0, -pos_factor * 0.0006 * math.cos(3*np.pi/2 - angle_rad)], [0, 1, 0, 0], [0, 0, 1, -pos_factor * 0.0006 * math.cos(angle_rad - np.pi)], [0, 0, 0, 1]])
        pcd.transform(translation_matrix)
    elif image_pos < 360:
        translation_matrix = np.array([[1, 0, 0, -pos_factor * 0.0006 * math.cos(angle_rad - 3*np.pi/2)], [0, 1, 0, 0], [0, 0, 1, pos_factor * 0.0006 * math.cos(2*np.pi - angle_rad)], [0, 0, 0, 1]])
        pcd.transform(translation_matrix)
