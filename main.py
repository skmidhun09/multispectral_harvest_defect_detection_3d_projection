...
def estimate_depth_glpn(imagePath, imageName):
    ...
    depth_image.save("temp/depth/" + imageName)
    image.save("temp/img/" + imageName)
    print("GLPN Time taken for ", imageName, ": ", (datetime.now() - glpn_start).total_seconds(), "s")


#############################
def create_pcd(image_num, image_ext, sidecount, total, pcds, lock):
    pcs_start = datetime.now()
    image = Image.open("temp/img/" + image_num + image_ext)
    width, height = image.size
    depth_image = Image.open("temp/depth/" + image_num + image_ext)
...
def main_func():
    ...
    pcds = []
    global input_range
    image_count = input_range[1] - input_range[0]
    if __name__ == "__main__":
        ...
        for i in range(input_range[0], input_range[1]):
            #print(i)
            estimate_depth_glpn(input_path, str(i) + ".jpg")
            # thread = threading.Thread(target=estimate_depth_glpn, args=(input_path, str(i) + ".jpg",))
            # thread.start()
        ...
        lock = threading.Lock()
        initiated = False
        for j in range(input_range[0], input_range[1]):
            global side_count
            # thread_list1[thread_cnt].join()
            # thread_cnt = thread_cnt + 1
            if not initiated:
                tpcd_start = datetime.now()
                initiated = True
            create_pcd(str(j), ".jpg", side_count, image_count, pcds, lock)
        ...
        # print("Total GLPN Time taken :", (datetime.now() - tglpn_start).total_seconds(), "s")
        tglpn_start = datetime.now()
        # for k in range(0, len(thread_list2)):
        #     thread_list2[k].join()
    ...
    o3d.io.write_triangle_mesh("image" + str(image_count) + ".ply", object3d)
